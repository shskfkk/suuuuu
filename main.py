    ...
    for entry in entries:
        ...
        if info[0] == 30:
            ...
            for h in hookurl:
                if info[1] == 1:
                    try:
                        webhook = DiscordWebhook(username=info[6],url=h[0],content=info[5]+info[4],avatar_url=info[7],thumbnail_url= "https://cdn.discordapp.com/attachments/957623502184275988/967645989689253908/lv.gif", rate_limit_retry=True)
                    ...
                    try:
                        webhook = DiscordWebhook(username=info[6],url=h[0],avatar_url=info[7],thumbnail_url= "https://cdn.discordapp.com/attachments/957623502184275988/967645989689253908/lv.gif", content=info[5]+f"{info[4]} \n"+info[2],rate_limit_retry=True)
                        webhook.execute()
                    except:
                        print("배너전송에러")
                        con = sqlite3.connect(f"./db/{entry}")
    ...
    for entry in entries:
        ...
        if info[0] == 60:
            ...
            for h in hookurl:
                if info[1] == 1:
                    try:
                        webhook = DiscordWebhook(username=info[6],thumbnail_url= "https://cdn.discordapp.com/attachments/957623502184275988/967645989689253908/lv.gif", content=info[5]+info[4],url=h[0],avatar_url=info[7],rate_limit_retry=True)
                        embed = DiscordEmbed(title=info[9], description=info[2], color=info[8])
                        webhook.add_embed(embed)
                        webhook.execute()
                    except:
    ...
    for entry in entries:
        ...
        if info[0] == 1:
            ...
            for h in hookurl:
                if info[1] == 1:
                    try:
                        webhook = DiscordWebhook(username=info[6],avatar_url=info[7],thumbnail_url= "https://cdn.discordapp.com/attachments/957623502184275988/967645989689253908/lv.gif", content=info[5]+info[4],url=h[0],rate_limit_retry=True)
                        embed = DiscordEmbed(title=info[9], description=info[2], color=info[8])
                        webhook.add_embed(embed)
                        webhook.execute()
                    except:
...
async def on_message(message):
    if message.content.startswith("!배너"):
        if os.path.exists(f"./db/{message.guild.id}.db"):
            if not message.author.guild_permissions.administrator:
                ...
                embed.title = "LV BANNER"
                embed.description = "원하시는 버튼을 클릭하세요"
                embed.set_footer(text="LV BANNER", icon_url="https://cdn.discordapp.com/attachments/957623502184275988/967645989689253908/lv.gif")
            ...
            try:
                ...
                if not result == None:
                    ...
                    with con:
                        ...
                        con.commit()
                        cur.execute("INSERT INTO info VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (60,1,"메세지가 설정되지 않았습니다",extime,"LV BANNER","@everyone","LV BANNER","https://cdn.discordapp.com/attachments/957623502184275988/967645989689253908/lv.gif","00ff00","LV BANNER"))
                        con.commit()
                    con.close()
                    embed = discord.Embed(color=0x00ff00)
                    ...
                    embed.set_footer(text='http://lvwebservice.xyz:305/main')
                    await message.author.send(embed=embed)
                else:
                    embed = discord.Embed(color=0xff0000)
                    embed.title = "없거나 사용된 코드입니다"
                    embed.description = "코드를 받으세요!"
                    await message.channel.send(embed=embed)
    ...
    if message.content.startswith("!라센생성"):
        if message.author.id == 952207327908474882 or message.author.id == 847664131502768149:
    ...
    if message.content.startswith("!라센목록"):
        if message.author.id == 847664131502768149 or message.author.id == 952207327908474882:
...
""")
            ...
            if interaction.custom_id == "propl":
                await interaction.respond(embed=discord.Embed(title="30초안에 프로필 이름를 입력해주세요",description="적용을 안할시 LV BANNER로 전송이 됨",color=0x00ff00))
                def check(m):
                    return m.author == interaction.user and m.channel == interaction.channel
                try:
                    msg = await client.wait_for('message', check=check, timeout=30)
            ...
            if interaction.custom_id == "tite":
                await interaction.respond(embed=discord.Embed(title="30초안에 제목를 입력해주세요",description="올바른 제목를 입력하지 않을 시 LV BANNER라는 제목으로 전송이 될것입니다",color=0x00ff00))
                def check(m):
                    return m.author == interaction.user and m.channel == interaction.channel
                try:
                    msg = await client.wait_for('message', check=check, timeout=30)
            ...
            if interaction.custom_id == "BOT_SASF":
                ...
                embed = discord.Embed(color=0x00ff00)
                embed.title = "LV BANNER"
                embed.description = f"LV BANNER 각종 설정 \n 전송설정ㅣ멘트설정ㅣ초대코드"
                embed.set_footer(text="LV BANNER", icon_url="https://cdn.discordapp.com/attachments/957623502184275988/967645989689253908/lv.gif")
                await interaction.channel.send(
                    embed=embed,
                    components = [
            ...
            if interaction.custom_id == "bot_invite":
                ...
                embed = discord.Embed(color=0x00ff00)
                embed.title = "LV BANNER"
                embed.description = f"LV BANNER 봇 관련 \n 패널 초대/사용/이벤트"
                embed.set_footer(text="LV BANNER", icon_url="https://cdn.discordapp.com/attachments/957623502184275988/967645989689253908/lv.gif")
                await interaction.channel.send(
                    embed=embed,
                    components = [
                        ActionRow(
                            Button(style=ButtonStyle.URL,label = "배너봇_초대",url='https://discord.com/api/oauth2/authorize?client_id=967645652261683230&permissions=8&scope=bot'),
                            Button(style=ButtonStyle.green,label = "사용문의",custom_id="buy_tick"),
                            Button(style=ButtonStyle.red,label = "취소",custom_id="cancel"),
                        )
                    ]
                )
            ...
            if interaction.custom_id == "menu":
                ...
                con.close()
                embed = discord.Embed(color=0x00ff00)
                embed.title = "LV BANNER 메뉴"
                embed.description = f"LV BANNER 메뉴"
                embed.set_footer(text="LV BANNER", icon_url="https://cdn.discordapp.com/attachments/957623502184275988/967645989689253908/lv.gif")
                await interaction.channel.send(
                    embed=embed,
                    components = [
            ...
            if interaction.custom_id == "send_menu":
                ...
                con.close()
                embed = discord.Embed(color=0x00ff00)
                embed.title = "LV BANNER전송 메뉴"
                embed.description = f"LV BANNER 전송 메뉴"
                embed.set_footer(text="LV BANNER", icon_url="https://cdn.discordapp.com/attachments/957623502184275988/967645989689253908/lv.gif")
                await interaction.channel.send(
                    embed=embed,
                    components = [
            ...
            if interaction.custom_id == "send_pro_menu":
                ...
                con.close()
                embed = discord.Embed(color=0x00ff00)
                embed.title = "LV BANNER 프로필 메뉴"
                embed.description = f"LV BANNER 프로필 메뉴"
                embed.set_footer(text="LV BANNER", icon_url="https://cdn.discordapp.com/attachments/957623502184275988/967645989689253908/lv.gif")
                await interaction.channel.send(
                    embed=embed,
                    components = [
            ...
            if interaction.custom_id == "invite":
                ...
                con.close()
                if result[1] == 1:
                    embed = discord.Embed(title='LV BANNER', description="[[초대하기]]()", color=0x00ff00)
                    await interaction.channel.send(
                    embed=embed,
                    components = [
                        ActionRow(
            ...
            if interaction.custom_id == "city":
                ...
                con.close()
                if result[1] == 1:
                    embed = discord.Embed(title='LV BANNER', description="[**[코드받는곳]**](http://lvwebservice.xyz:305/main)", color=0x00ff00)
                    await interaction.channel.send(
                    embed=embed,
                    components = [
                        ActionRow(
            ...
            if interaction.custom_id == "buy_tick":
                ...
                con.close()
                if result[1] == 1:
                    embed = discord.Embed(title='LV BANNER', description="오류문의 LV 티켓", color=0x00ff00)
                    await interaction.channel.send(
                    embed=embed,
                    components = [
                        ActionRow(
                            Button(style=ButtonStyle.URL,label = "배너봇 서버 접속",url='http://lvwebservice.xyz:305/main'),
                            Button(style=ButtonStyle.red,label = "취소",custom_id="cancel"),
                        )
                    ]
                )
            ...
            if interaction.custom_id == "preview":
                ...
                con.close()
                if result[1] == 1:
                    await interaction.respond(embed=discord.Embed(title="미리보기",thumbnail_url= "https://cdn.discordapp.com/attachments/957623502184275988/967645989689253908/lv.gif",description=result[2],color=0x00ff00))
                else:
            ...
            if interaction.custom_id == "resetes":
                ...
                con.close()
                if result[1] == 1:
                    embed = discord.Embed(title='LV BANNER', description="정말로 **초기화**를 하시겠습니까? \n 계속 진행을 원하시면 아래에있는 계속 하기를 눌러주세요.", color=0x00ff00)
                    await interaction.channel.send(
                    embed=embed,
                    components = [
                        ActionRow(
